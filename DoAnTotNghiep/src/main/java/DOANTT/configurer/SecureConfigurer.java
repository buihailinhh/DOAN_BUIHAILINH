...
import DOANTT.service.UserDetailsServiceImpl;
...
public class SecureConfigurer extends WebSecurityConfigurerAdapter {
	...
	protected void configure(final HttpSecurity http) throws Exception {
		...
		.antMatchers("/customer/**", "/FileUpload/**", "/admin/**","/user/**", "/login", "/logout").permitAll()
...
//.defaultSuccessUrl("/manager/home-admin", true) //login thanh cong thi tro ve trang home.jsp
		...
		.successHandler(new UrlAuthenticatonSuccessHandler())// chuyển đến request phù hợp vs role
		...
		.logout().logoutUrl("/logout").logoutSuccessUrl("/home/1").invalidateHttpSession(true)
...
//			 System.out.println(new BCryptPasswordEncoder(4).encode("664422")); 
//			
//		   }
//		 
}
